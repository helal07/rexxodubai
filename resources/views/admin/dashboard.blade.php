...
<head>
    ...
    <!-- Dynamic Favicon & Safe View Data -->
    @php
...
<body class="bg-[#f8fafc] text-[#0f172a] font-sans flex flex-col min-h-screen relative overflow-x-hidden selection:bg-[#4338ca] selection:text-white">
    ...
    <div class="flex flex-1 w-full min-h-0 relative overflow-hidden">
        ...
        @php
            $profileUser = $currentUser ?? Auth::user();
        @endphp
        <div id="section-profile" class="section-content hidden space-y-6">
            <!-- Header Bar -->
            <div class="bg-white/90 border border-slate-200 p-6 rounded-2xl shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#4338ca] to-indigo-700 text-white flex items-center justify-center shadow-md shadow-indigo-500/20 shrink-0">
                        <i data-lucide="user" class="w-6 h-6"></i>
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-[#4338ca] bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-100">ACCOUNT SETTINGS</span>
                            <span class="text-[10px] text-slate-500 font-mono">AUTHENTICATED USER</span>
                        </div>
                        <h2 class="text-[18px] font-serif font-bold text-[#0f172a] uppercase">My Profile & Contact Information</h2>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" onclick="switchSection('profile_password')" class="px-3.5 py-2 text-[12px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-xl hover:bg-amber-100 transition-colors flex items-center gap-1.5 cursor-pointer">
                        <i data-lucide="key" class="w-3.5 h-3.5"></i> Change Password
                    </button>
                    <button type="button" onclick="switchSection('users_management')" class="px-3.5 py-2 text-[12px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl hover:bg-emerald-100 transition-colors flex items-center gap-1.5 cursor-pointer">
                        <i data-lucide="users" class="w-3.5 h-3.5"></i> Manage Users
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="bg-white/90 rounded-2xl border border-slate-200 p-6 text-center shadow-xs relative overflow-hidden">
                        <div class="h-16 -mx-6 -mt-6 bg-gradient-to-r from-[#4338ca] to-sky-500 mb-0"></div>
                        <div class="relative -mt-9 mb-4 inline-block">
                            <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md mx-auto bg-slate-100 flex items-center justify-center">
                                <img id="dashProfileAvatarPreview" src="{{ $profileUser->avatar_url ?? $profileUser->avatar }}" alt="{{ $profileUser->name }}" class="w-full h-full object-cover" />
                            </div>
                            <label for="dash_avatar_file_input" class="absolute bottom-0 right-0 w-7 h-7 rounded-full bg-[#4338ca] hover:bg-indigo-700 text-white flex items-center justify-center cursor-pointer shadow-md transition-all" title="Upload New Photo">
                                <i data-lucide="camera" class="w-3.5 h-3.5"></i>
                            </label>
                        </div>

                        <h3 class="text-base font-extrabold text-slate-900 leading-tight">{{ $profileUser->name }}</h3>
                        <p class="text-xs text-slate-500 font-mono mt-0.5">{{ $profileUser->email }}</p>

                        <div class="mt-3 flex items-center justify-center gap-2">
                            <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-indigo-50 text-[#4338ca] border border-indigo-100 inline-flex items-center gap-1.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                {{ $profileUser->role ?? ($profileUser->is_admin ? 'Super Administrator' : 'Staff') }}
                            </span>
                        </div>

                        <div class="mt-5 pt-4 border-t border-slate-100 grid grid-cols-2 gap-2 text-left">
                            <div class="bg-slate-50 p-2.5 rounded-xl">
                                <p class="text-[10px] uppercase font-bold text-slate-400">Phone</p>
                                <p class="text-xs font-bold text-slate-800 truncate mt-0.5">{{ $profileUser->phone ?? 'Not set' }}</p>
                            </div>
                            <div class="bg-slate-50 p-2.5 rounded-xl">
                                <p class="text-[10px] uppercase font-bold text-slate-400">Country</p>
                                <p class="text-xs font-bold text-slate-800 truncate mt-0.5">{{ $profileUser->country ?? 'Bangladesh' }}</p>
                            </div>
                        </div>
                        ...
                        @if(!empty($profileUser->avatar))
                            <form action="{{ url('/admin/profile/remove-avatar') }}" method="POST" class="mt-4">
                                @csrf
                                <button type="submit" onclick="return confirm('Remove your custom avatar photo?')" class="text-xs font-bold text-rose-600 hover:text-rose-700 hover:underline flex items-center justify-center gap-1 mx-auto cursor-pointer">
                ...
                <div class="lg:col-span-8">
                    <div class="bg-white/90 rounded-2xl border border-slate-200 p-6 shadow-xs">
                        <h3 class="text-[14px] font-bold text-[#0f172a] uppercase border-l-4 border-[#4338ca] pl-2 mb-4">Edit Personal Information</h3>
                        <form action="{{ url('/admin/profile/update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div class="p-3 bg-slate-50 rounded-xl border border-dashed border-slate-300 flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full overflow-hidden bg-white border border-slate-200 shrink-0">
                                    <img id="dashAvatarFormPreview" src="{{ $profileUser->avatar_url ?? $profileUser->avatar }}" class="w-full h-full object-cover">
                                </div>
                                <div class="flex-1">
                                    <label class="text-xs font-bold text-slate-900 block">Change Avatar Photo</label>
                                    <input type="file" name="avatar_file" id="dash_avatar_file_input" accept="image/*" onchange="previewDashAvatar(event)" class="text-xs text-slate-600 file:mr-3 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-[#ede9fe] file:text-[#4338ca] cursor-pointer">
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Full Name <span class="text-rose-500">*</span></label>
                                    <input type="text" name="name" value="{{ $profileUser->name }}" required class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:border-[#4338ca] outline-none">
                                </div>
                                <div>
                                    <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Email Address <span class="text-rose-500">*</span></label>
                                    <input type="email" name="email" value="{{ $profileUser->email }}" required class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:border-[#4338ca] outline-none font-mono">
                                </div>
                                <div>
                                    <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Phone Number</label>
                                    <input type="text" name="phone" value="{{ $profileUser->phone }}" placeholder="+880 1700-000000" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:border-[#4338ca] outline-none font-mono">
                                </div>
                                <div>
                                    <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Role / Designation Title</label>
                                    <input type="text" name="designation" value="{{ $profileUser->designation }}" placeholder="e.g. Chief Executive / Operations Lead" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:border-[#4338ca] outline-none">
                                </div>
                            </div>

                            <div class="space-y-4 pt-1">
                                <div>
                                    <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Street Address</label>
                                    <textarea name="address" rows="2" placeholder="e.g. House #12, Road #5, Dhanmondi" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:border-[#4338ca] outline-none">{{ $profileUser->address }}</textarea>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">City / District</label>
                                        <input type="text" name="city" value="{{ $profileUser->city }}" placeholder="e.g. Dhaka" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:border-[#4338ca] outline-none">
                                    </div>
                                    <div>
                                        <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Country</label>
                                        <input type="text" name="country" value="{{ $profileUser->country ?? 'Bangladesh' }}" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 focus:border-[#4338ca] outline-none">
                                    </div>
                                </div>
                            </div>
        ...
        <!-- SECTION: USER SECURITY & PASSWORD -->
        <div id="section-profile_password" class="section-content hidden space-y-6">
            <div class="bg-white/90 border border-slate-200 p-6 rounded-2xl shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 text-white flex items-center justify-center shadow-md shadow-amber-500/20 shrink-0">
                        <i data-lucide="key" class="w-6 h-6"></i>
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-amber-700 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200">SECURITY CENTER</span>
                            <span class="text-[10px] text-slate-500 font-mono">CREDENTIALS UPDATE</span>
                        </div>
                        <h2 class="text-[18px] font-serif font-bold text-[#0f172a] uppercase">Change Password & Security Settings</h2>
                    </div>
                </div>
                <button type="button" onclick="switchSection('profile')" class="px-3.5 py-2 text-[12px] font-bold text-[#4338ca] bg-indigo-50 border border-indigo-200 rounded-xl hover:bg-indigo-100 transition-colors flex items-center gap-1.5 cursor-pointer">
                    <i data-lucide="user" class="w-3.5 h-3.5"></i> Back to Profile
                </button>
            </div>

            <div class="max-w-2xl bg-white/90 rounded-2xl border border-slate-200 p-6 shadow-xs space-y-5">
                <form action="{{ url('/admin/profile/password') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Current Password <span class="text-rose-500">*</span></label>
                        <input type="password" name="current_password" required placeholder="••••••••" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:border-[#4338ca] outline-none">
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">New Password <span class="text-rose-500">*</span></label>
                            <input type="password" name="password" required placeholder="Minimum 6 chars" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:border-[#4338ca] outline-none">
                        </div>
                        <div>
                            <label class="text-[11px] font-bold uppercase text-slate-700 block mb-1">Confirm New Password <span class="text-rose-500">*</span></label>
                            <input type="password" name="password_confirmation" required placeholder="Repeat password" class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:border-[#4338ca] outline-none">
                        </div>
                    </div>
                    <div class="pt-3 flex justify-end">
        ...
        <!-- SECTION: ADMIN USERS & STAFF MANAGEMENT -->
        <div id="section-users_management" class="section-content hidden space-y-6">
            <div class="bg-white/90 border border-slate-200 p-6 rounded-2xl shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 text-white flex items-center justify-center shadow-md shadow-emerald-500/20 shrink-0">
                        <i data-lucide="users" class="w-6 h-6"></i>
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="text-[10px] font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">USER MANAGEMENT</span>
                            <span class="text-[10px] text-slate-500 font-mono">ADMIN & STAFF ACCOUNTS</span>
                        </div>
                        <h2 class="text-[18px] font-serif font-bold text-[#0f172a] uppercase">Admin Users & Staff Accounts</h2>
                    </div>
                </div>
                <button type="button" onclick="openAddUserModal()" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer shadow-xs">
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-200">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                            @if(isset($users))
                                @foreach($users as $u)
                                    <tr class="hover:bg-slate-50/80 transition-colors {{ $u->id === ($profileUser->id ?? 0) ? 'bg-indigo-50/30' : '' }}">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
    ...
    <script>
        ...
        const parentSubmenuMap = {
            ...
            'api_settings': 'api_gateway',
            'courier_setup': 'api_gateway',
            'courier_send': 'api_gateway',
            'courier_history': 'api_gateway',
            'seo': 'seo_sub',
            'seo_meta': 'seo_sub',
            ...
            'seo_ping': 'seo_sub',
            'profile': 'user_mgmt',
            'profile_password': 'user_mgmt',
            'users_management': 'user_mgmt'
        };

        function previewDashAvatar(event) {
            const input = event.target;
            if (input.files && input.files[0]) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    const preview1 = document.getElementById('dashProfileAvatarPreview');
                    const preview2 = document.getElementById('dashAvatarFormPreview');
        ...
        @php
            $formattedProducts = isset($products) ? collect($products)->map(function($p) {
                return [
                    'id' => $p->id,
                    'name' => $p->name,
                    'slug' => $p->slug,
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            // Show global success toast if session has success message
            @if(session('success'))
            showToastNotice({!! json_encode(session('success')) !!});
            @endif
        });
