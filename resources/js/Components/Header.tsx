...
import { ShoppingBag, Search, Menu, X, Phone, Globe, Mail, MessageSquare, MessageCircle, Headset } from 'lucide-react';
...
export default function Header({ initialMenu }: HeaderProps) {
  ...
  const activeMenuItems = (initialMenu && initialMenu.length > 0 && initialMenu !== FALLBACK_MENU) ? initialMenu : ((menuItems && menuItems.length > 0) ? menuItems : FALLBACK_MENU);
  ...
  const isSolidHeader = pathname !== '/' || isScrolled || isHeaderHovered || searchOpen;

  return (
    <>
      {/* Dynamic Announcement Bar if present */}
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6 sm:space-x-8 flex-1">
            <button
              ...
              className="flex items-center gap-2 text-[14px] font-normal tracking-wide text-[#0A0A0A] hover:opacity-70 transition-opacity cursor-pointer shrink-0"
          ...
          {/* Dynamic Brand Logo / Wordmark Automatically Synced with Admin Panel Settings */}
          <div className="shrink-0 text-center px-4">
            ...
            >
              {(settings.logo_url || settings.site_logo) ? (
                ...
                <span className="font-serif text-[26px] md:text-[34px] tracking-[0.16em] font-extrabold uppercase text-[#0A0A0A] leading-none transition-transform group-hover:tracking-[0.18em]">
                  {settings.siteName || 'PRADA'}
                </span>
              )}
            </Link>
          </div>

          {/* Right Actions: Contact us */}
          <div className="flex items-center space-x-6 flex-1 justify-end shrink-0">
            <button
              ...
              className="text-[14px] font-normal tracking-wide text-[#0A0A0A] hover:opacity-70 transition-opacity cursor-pointer"
            >
              Contact us
            </button>
          </div>
        ...
        {/* Luxury Prada-Style Full-Width Search Overlay */}
        {searchOpen && (
          <div className="bg-white text-[#0A0A0A] border-t border-[#DEDBD4] py-6 px-8 shadow-xl animate-fade-in">
            <div className="max-w-3xl mx-auto space-y-4">
              <div className="flex items-center border-b-2 border-[#0A0A0A] pb-2">
                ...
                <input
                  type="text"
                  placeholder="Search by perfume name, amber, oud, rose, notes..."
                  value={searchQuery}
                ...
                <button
                  ...
                  className="bg-[#0A0A0A] text-white text-[11px] font-bold uppercase tracking-widest px-5 py-2 hover:bg-[#B8712E] transition-colors shrink-0"
                >
                  SEARCH
                </button>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1 text-[12px] text-[#6E6B66]">
                ...
                {['Amber', 'Woody Leather', 'Oud', 'Gifts', 'Rose'].map(tag => (
                  <button
                    key={tag}
                    onClick={() => {
      ...
      {/* 1:1 Prada Left Slide-In Drawer Navigation */}
      {menuDrawerOpen && (
        <div className="fixed inset-0 z-50 flex animate-fade-in">
          {/* Dark Overlay Background for Right Side */}
          <div
          ...
          <div className="relative w-full max-w-[360px] bg-white text-[#0A0A0A] h-full flex flex-col justify-between p-8 shadow-2xl z-10 overflow-y-auto">
            {/* Top Bar: Close & Search */}
            <div className="flex justify-between items-center pb-8 border-b border-[#F5F3EF]">
              <button
                onClick={() => setMenuDrawerOpen(false)}
                className="flex items-center gap-2 text-[14px] font-medium text-[#0A0A0A] hover:opacity-70 transition-opacity"
              >
                <X size={18} />
                <span>Close</span>
              </button>
            ...
            {/* Main Navigation Links List (Dynamically synced from Backend API & MySQL DB) */}
            <div className="py-8 flex-1 space-y-6">
              <div className="space-y-4">
                {activeMenuItems.map((item) => (
                  <Link
                    key={item.id}
                    href={item.url || '/perfumes'}
                    onClick={() => setMenuDrawerOpen(false)}
                    className="block text-[15px] font-sans font-medium text-[#0A0A0A] hover:font-bold hover:text-[#B8712E] transition-all uppercase"
                  >
                    {item.label}
                  </Link>
                ))}
      ...
      {/* 1:1 Prada Right Slide-In Client Service Drawer (Matching User Screenshot 100%) */}
      {contactDrawerOpen && (
        <div className="fixed inset-0 z-50 flex justify-end animate-fade-in">
          {/* Dark Overlay Background for Left Side */}
          <div
            className="fixed inset-0 bg-black/65 backdrop-blur-xs transition-opacity"
          ...
          <div className="relative w-full max-w-[480px] bg-white text-[#0A0A0A] h-full flex flex-col justify-between p-8 md:p-10 shadow-2xl z-10 overflow-y-auto">
            ...
            <div className="flex justify-end">
              <button
                onClick={() => setContactDrawerOpen(false)}
                className="p-2 text-[#0A0A0A] hover:opacity-60 transition-opacity"
            ...
            <div className="py-4 space-y-8 flex-1">
              ...
              {/* Stacked Option Cards (Direct WhatsApp & Gmail Message Options) */}
              <div className="space-y-4">
              ...
              {/* Bottom Policy & Hours Description Note */}
