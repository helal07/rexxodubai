...
export default function LandingPageEdit({ landingPage, availableProducts }: any) {
    const { data, setData, processing, errors } = useForm({
        ...
        regular_price: landingPage.regular_price || '',
        offer_price: landingPage.offer_price || '',
        offer_end_date: landingPage.offer_end_date ? new Date(landingPage.offer_end_date).toISOString().slice(0, 16) : '',
        
        theme_color: landingPage.theme_color || '#4f46e5',
        text_color: landingPage.text_color || '#1e293b',
        background_color: landingPage.background_color || '#f8fafc',
        other_color: landingPage.other_color || '#ffffff',
        
        primary_button_text: landingPage.primary_button_text || 'Buy Now',
        secondary_button_text: landingPage.secondary_button_text || 'Learn More',
        youtube_video_url: landingPage.youtube_video_url || '',
        youtube_autoplay: landingPage.youtube_autoplay ?? true,
        
        assigned_products: landingPage.products?.map((p: any) => ({
            id: p.id,
            regular_price: p.pivot?.regular_price ?? (p.price || ''),
            offer_price: p.pivot?.offer_price ?? '',
        })) || [],
        
        features: landingPage.features?.length ? landingPage.features : [
            { title: '', text: '' },
            { title: '', text: '' },
        ],
        why_choose_us: landingPage.why_choose_us?.length ? landingPage.why_choose_us : [
            { title: '', text: '' },
            { title: '', text: '' },
            { title: '', text: '' },
            { title: '', text: '' },
        ],
        media_banners: landingPage.media_banners?.length ? landingPage.media_banners : ['', ''],
        reviews: landingPage.reviews?.length ? landingPage.reviews : [
            { name: '', city: '', rating: '5', text: '' },
            { name: '', city: '', rating: '5', text: '' },
            { name: '', city: '', rating: '5', text: '' },
            { name: '', city: '', rating: '5', text: '' },
        ],
        gallery_images: landingPage.gallery_images?.length ? landingPage.gallery_images : ['', '', '', '', '', '', '', ''],
        
        short_description: landingPage.short_description || '',
        long_description: landingPage.long_description || '',
        
        faqs: landingPage.faqs?.length ? landingPage.faqs : [
            { question: '', answer: '' },
            { question: '', answer: '' },
            { question: '', answer: '' },
        ],
        
        homepage_product_title: landingPage.homepage_product_title || 'Featured Product',
        show_product_section: landingPage.show_product_section ?? true,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Use router.post with _method: 'put' for multipart file uploads on update in Laravel
        router.post(`/admin/landing-pages/${landingPage.id}`, {
            _method: 'put',
            ...data,
        }, {
            forceFormData: true,
    ...
    return (
        <AdminLayout activePage="landing-pages" pageTitle={`Edit Landing Page: ${landingPage.title}`}>
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-bold text-[#0f172a]">Edit Landing Page</h2>
                </div>
                <div className="flex items-center gap-3">
                    <button 
                        type="button" 
